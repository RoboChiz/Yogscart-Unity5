...
Shader "Yogscart/GenericSplatShaderWorkAround2" {
    Properties {
        ...
        _SkyboxIntensity ("Skybox Intensity", Range(0, 1)) = 0
        _GrassOverlay ("GrassOverlay", 2D) = "white" {}
        _OverlayTexStrength ("Overlay Tex Strength", Range(0, 0.2)) = 0.2
        _OverlayTexRotate ("Overlay Tex Rotate", Range(0, 6.2)) = 2.967521
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform sampler2D _GrassOverlay; uniform float4 _GrassOverlay_ST;
            uniform float _OverlayTexRotate;
...
////// Specular:
                ...
                float node_6779_ang = _OverlayTexRotate;
                float node_6779_spd = 1.0;
                float node_6779_cos = cos(node_6779_spd*node_6779_ang);
                float node_6779_sin = sin(node_6779_spd*node_6779_ang);
                float2 node_6779_piv = float2(0.5,0.5);
                float2 node_6779 = (mul(i.uv0-node_6779_piv,float2x2( node_6779_cos, -node_6779_sin, node_6779_sin, node_6779_cos))+node_6779_piv);
                float4 _GrassOverlay_var = tex2D(_GrassOverlay,TRANSFORM_TEX(node_6779, _GrassOverlay));
                float3 diffuseColor = (lerp(_Albedo1_var.rgb,_Albedo2_var.rgb,saturate((1.0-(_SplatMap_var.r-step(_SplatMap_var.r,_HeightMap_var.rgb)))))*(1.0-(_GrassOverlay_var.rgb*_OverlayTexStrength))); // Need this for specular when using metallic
...
/////// Vectors:
                ...
                float2 node_6779 = (mul(i.uv0-node_6779_piv,float2x2( node_6779_cos, -node_6779_sin, node_6779_sin, node_6779_cos))+node_6779_piv);
                float4 _GrassOverlay_var = tex2D(_GrassOverlay,TRANSFORM_TEX(node_6779, _GrassOverlay));
                float3 diffColor = (lerp(_Albedo1_var.rgb,_Albedo2_var.rgb,saturate((1.0-(_SplatMap_var.r-step(_SplatMap_var.r,_HeightMap_var.rgb)))))*(1.0-(_GrassOverlay_var.rgb*_OverlayTexStrength)));
                float specularMonochrome;
                float3 specColor;
                float2 node_1371 = (i.uv0*_MetallicTileFactor);
                float4 _Metallic_var = tex2D(_Metallic,TRANSFORM_TEX(node_1371, _Metallic));
