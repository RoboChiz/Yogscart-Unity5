...
Shader "Shader Forge/SnowShader" {
    Properties {
        ...
        _Fresnel ("Fresnel", Range(0, 15)) = 4.746665
        _FresnelColor ("Fresnel Color", Color) = (0.9206897,0.9206897,0.9206897,1)
        _LightWrapping ("Light Wrapping", Range(0, 0.5)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 _DetailNormalMap_var = tex2D(_DetailNormalMap,TRANSFORM_TEX(i.uv0, _DetailNormalMap));
                float3 node_8754_nrm_base = _NormalMap_var.rgb + float3(0,0,1);
                float3 node_8754_nrm_detail = _DetailNormalMap_var.rgb * float3(-1,-1,1);
                float3 node_8754_nrm_combined = node_8754_nrm_base*dot(node_8754_nrm_base, node_8754_nrm_detail)/node_8754_nrm_base.z - node_8754_nrm_detail;
                float3 node_8754 = node_8754_nrm_combined;
                float3 normalLocal = node_8754;
...
/////// Diffuse:
                ...
                float3 backLight = pow(max(float3(0.0,0.0,0.0), -NdotLWrap + w ), _Diffuse_var.rgb) * float3(_TransmissionAmount,_TransmissionAmount,_TransmissionAmount);
