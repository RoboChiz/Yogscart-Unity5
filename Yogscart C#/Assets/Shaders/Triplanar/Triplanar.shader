...
Shader "Triplanar/Naive Method"
{
    Properties
    {
		...
		_GrassSpread("Grass Spread", float) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				// Show Grass if valid
				if(saturate(dot(worldNormal, half3(0,1,0))) >= _GrassSpread)
				{
					colX = tex2D(_GrassTex, uvX);
					colY = tex2D(_GrassTex, uvY);
					colZ = tex2D(_GrassTex, uvZ);
				}
