...
public class InputController
{
    ...
    public InputController(string inputName)
    {
        ...
        if (inputName == "Key_")
            controlLayout = OldInputManager.AllConfigs[0];
        else
            controlLayout = OldInputManager.AllConfigs[1];
    ...
    private float actualGetInput(string axis, bool getRaw)
    {
        ...
        if (!OldInputManager.lockEverything)
        {
            ...
            if (!HasInputOne && !HasInputTwo && OldInputManager.menuInputs.Contains(axis))
            {
                //OH NO! Better check the Default Inputs
                HasInputOne = OldInputManager.splitConfigs[(int)controlLayout.Type][0].commandsOne.TryGetValue(axis, out inputAxisOne);
                HasInputTwo = OldInputManager.splitConfigs[(int)controlLayout.Type][0].commandsTwo.TryGetValue(axis, out inputAxisTwo);
            }
            ...
            else
            {
                if (inputAxisOne != null)
                {
                    if (controlLayout.Type == ControllerType.Xbox360)
                    {
                        ...
                        if (inputAxisOne.Length >= 5 && inputAxisOne.Substring(inputAxisOne.Length - 4, 4) == "_Inv")
                        {
                            invert = true;
                            inputAxisOne = inputAxisOne.Remove(inputAxisOne.Length - 4);
                        }

                        if (!getRaw)
                            value = (invert ? -1f : 1f) * Input.GetAxis(inputAxisOne + controllerName);
                        else
                            value = (invert ? -1f : 1f) * Input.GetAxisRaw(inputAxisOne + controllerName);
                ...
                if ((inputAxisTwo != null) && value == 0)
                {
                    if (controlLayout.Type == ControllerType.Xbox360)
                    {
                        ...
                        if (inputAxisTwo.Length >= 5 && inputAxisTwo.Substring(inputAxisTwo.Length - 4, 4) == "_Inv")
                        {
                            invert = true;
                            inputAxisTwo = inputAxisTwo.Remove(inputAxisTwo.Length - 4);
                        }
    ...
    public bool LoadInput(string contents)
    {
        ...
        if (splitString.Length >= 2) //Must have 2
        {
            ...
            for (int i = 2; i < splitString.Length; i++)
            {
                ...
                if (inputSplit.Length == 2 && OldInputManager.validCommands.Contains(inputSplit[0]))
                {
